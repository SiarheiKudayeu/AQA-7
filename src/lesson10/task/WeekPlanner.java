...
public class WeekPlanner {
    ...
    public void workWithSchedule(String[][] schedule){
        ...
        while (!dayOfTheWeek.equalsIgnoreCase("exit")) {

            if(dayOfTheWeek.contains("change")||dayOfTheWeek.contains("reschedule")){
                //change Monday
                String day = dayOfTheWeek.split("\\s")[1];
                for(int i = 0; i < schedule.length; i++){
                    if(schedule[i][0].equalsIgnoreCase(day)){
                        System.out.println("Please insert new task for " + schedule[i][0]);
                        schedule[i][1] = scanner.nextLine();
                        break;
                    }
                }
